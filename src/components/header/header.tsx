...
export default function Header({ chatId }: HeaderProps) {
    ...
    const handleShareChat = async () => {
        try {
            const data = await fetch(`/api/chat/share?chatId=${currentChat}`, {
                method: 'GET'
            }).then(res => res.json());
            toast({
    ...
    const handleResetChat = async () => {
        try {
            const response = await fetch(`/api/chat/reset?chatId=${currentChat}`, {
                method: 'DELETE',
            }).then(() => {
                toast({
                    title: "chat reset successfull",
                });
    ...
    return (
        <div className="w-full fixed z-10 h-14 flex py-4 items-center justify-around px-10 border-b-[1px] border-neutral-700 bg-[#202026]">
            ...
            {currentChat && <div className="flex gap-2 absolute border w-full">
                <svg className="cursor-pointer" onClick={() => handleShareChat()} width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <svg className=" cursor-pointer" onClick={() => handleResetChat()} width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect x="0.5" y="0.5" width="39" height="39" rx="19.5" stroke="#373740" />
